...
#' rented.
#' 
#' Output: dataframe called income_2011
#' containing each source of income 
#' seperately and at the household level
...
# 
# # read in raw data
# off_farm_income <- read_dta(file.path(dataPath, "sect4_hh_w1.dta")) %>%
...
# off_farm_income <- transmute(off_farm_income, household_id, individual_id,
...
# -------------------------------------
#' on-farm income
#' income over the last 12 months from
#' crop sales and rented land
#' crop: income from crop sales
#' tree: income from permanent crop sales
#' rent: income from rented land
# -------------------------------------

# from crops
crop <- read_dta(file.path(dataPath, "sect11_ph_w1.dta")) %>%
...
# to the household as a measure of on farm income

on_farm_income_crop <- group_by(crop, household_id) %>%
  summarise(crop_value_hh=sum(value, na.rm=TRUE))

# from permanent crops (trees)
tree <- read_dta(file.path(dataPath, "/sect12_ph_w1.dta")) %>%
  select(holder_id, household_id, crop_code,
         sold_tree=ph_s12q06, sold_tree_kg=ph_s12q07, tree_value=ph_s12q08)

# summarise to the household level
on_farm_income_tree <- group_by(tree, household_id) %>%
  summarise(tree_value_hh=sum(tree_value, na.rm=TRUE))

# from rented land
rent <- read_dta(file.path(dataPath, "sect2_pp_w1.dta")) %>%
  select(holder_id, household_id, parcel_id, rented=pp_s2q10, rented12=pp_s2q11,
         fields_rented=pp_s2q12, rent_cash=pp_s2q13_a, rent_in_kind=pp_s2q13_b)

# combine cash and in kind rent
# where in-kind rent is estimated by
# the farmer
...
# summarise to the household level
on_farm_income_rent <- group_by(rent, household_id) %>%
...
income_2011 <- full_join(on_farm_income_crop, on_farm_income_rent)
income_2011 <- full_join(income_2011, on_farm_income_tree)

# sum across all sources of income to 
# get a single income variable
income_2011$income <- with(income_2011,
