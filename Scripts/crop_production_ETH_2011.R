...
crop_prod <- read_dta(file.path(dataPath, "sect4_pp_w1.dta")) %>%
...
crop_prod$parcel_id <- as.integer(crop_prod$parcel_id)
crop_prod$field_id <- as.integer(crop_prod$field_id)

# -------------------------------------
# read in the field area information
# --------------------------------------

land <- read_dta(paste(dataPath, "../../../Other/Plot_size/areas_eth_y1_imputed.dta", sep="/")) %>%  
  select(household_id=case_id, holder_id, parcel_id, field_id, area_farmer=area_sr, area_gps=area_gps_mi_50)
...
land$parcel_id <- as.integer(land$parcel_id)
land$field_id <- as.integer(land$field_id)
...
# 1. plot area (farmer reported) 
crop_prod_w <- select(crop_prod, household_id, type, area_farmer)
crop_prod_area_farmer <- group_by(crop_prod_w, household_id, type) %>%
...
names(crop_prod_area_farmer)[1] <- "household_id"

# 2. plot area (gps reported, 25% of plots)
crop_prod_x <- select(crop_prod, household_id, type, area_gps)
crop_prod_area_gps <- group_by(crop_prod_x, household_id, type) %>%
  summarise_each(funs(sum)) %>% spread(key = type, value = area_gps) 
names(crop_prod_area_gps) <- paste0(names(crop_prod_area_gps), "_area_gps")
names(crop_prod_area_gps)[1] <- "household_id"

# 3. plot area (farmer + gps measured)
crop_prod_y <- select(crop_prod, household_id, type, area_mix)
crop_prod_area_mix <- group_by(crop_prod_y, household_id, type) %>%
  summarise_each(funs(sum)) %>% spread(key = type, value = area_mix) 
names(crop_prod_area_mix) <- paste0(names(crop_prod_area_mix), "_area_mix")
names(crop_prod_area_mix)[1] <- "household_id"
