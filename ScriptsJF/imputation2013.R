#'Multiple imputation code: obtain 2013 GPS areas through multiple imputation as proposed 
...
flds         <- read.dta("Data/Ethiopia 2013/Post-Planting/sect3_pp_w2.dta")
...
field_area <- dplyr::select(flds, household_id, parcel_id, field_id, plot_id, holder_id, irrig=pp_s3q12, fert=pp_s3q14, pop_weight=pw2,
                            region=saq01, zone=saq02, woreda=saq03, area=pp_s3q02_a, local_unit=pp_s3q02_c, gps=pp_s3q05_a)
...
#Creating imputation data frame with self-reported areas and gps areas in square meter units
...
#----------------------------
hh11         <- read.dta("Data/Ethiopia 2013/Household/sect1_hh_w2.dta")
edu          <- read.dta("Data/Ethiopia 2013/Household/sect2_hh_w2.dta")
...
rural$rural_residence[rural$setting!="Rural"] <- 0
...
household_characteristics <- merge(household_characteristics, rural)[-1,]
...
television <- arrange(television, household_id)[1487:5262,]
refrigerator <- dplyr::filter(assetdata, hh_s10q00=="Refridgerator")
refrigerator <- arrange(refrigerator, household_id)[1487:5262,]
phone <- dplyr::filter(assetdata, hh_s10q00=="Mobile telephone")
phone <- arrange(phone, household_id)[1487:5262,]
car <- dplyr::filter(assetdata, hh_s10q00=="Private car")
car <- arrange(car, household_id)[1487:5262,]
bicycle <- dplyr::filter(assetdata, hh_s10q00=="Bicycle")
bicycle <- arrange(bicycle, household_id)[1487:5262,]

wealthindex <- data.frame(household_id = unique(assetdata$household_id))
wealthindex = as.data.frame(arrange(wealthindex)[-1,])
names(wealthindex) <- c("household_id")
...
houseindicators <- dplyr::select(housing, household_id, rooms = hh_s9q04, floor = hh_s9q07, toilet=hh_s9q10, watersource = hh_s9q13, electricity=hh_s9q20_a) 
houseindicators <- arrange(houseindicators, household_id)[1487:5262,]
...
asset_cart <- arrange(asset_cart, household_id)[1487:5262,]
asset_animalcart <- dplyr::filter(assetdata, hh_s10q00=="Cart (animal drawn)")
asset_animalcart <- arrange(asset_animalcart, household_id)[1487:5262,]
asset_sickle     <- dplyr::filter(assetdata, hh_s10q00=="Sickle (Machid)")
asset_sickle <- arrange(asset_sickle, household_id)[1487:5262,]
asset_axe        <- dplyr::filter(assetdata, hh_s10q00=="Axe (Gejera)")
asset_axe <- arrange(asset_axe, household_id)[1487:5262,]
asset_pickaxe    <- dplyr::filter(assetdata, hh_s10q00=="Pick Axe (Geso)")
asset_pickaxe <- arrange(asset_pickaxe, household_id)[1487:5262,]
asset_plough     <- dplyr::filter(assetdata, hh_s10q00=="Plough (traditional)")
asset_plough <- arrange(asset_plough, household_id)[1487:5262,]
asset_ploughmod  <- dplyr::filter(assetdata, hh_s10q00=="Plough (modern)")
asset_ploughmod <- arrange(asset_ploughmod, household_id)[1487:5262,]
asset_waterpump  <- dplyr::filter(assetdata, hh_s10q00=="Water Pump")
asset_waterpump <- arrange(asset_waterpump, household_id)[1487:5262,]

implementindex <- data.frame(household_id = unique(assetdata$household_id))
implementindex <- as.data.frame(arrange(implementindex, household_id))
implementindex <- as.data.frame(implementindex[-1,])
names(implementindex) <- c("household_id")
...
off_farm_income <- arrange(off_farm_income, household_id)
off_farm_income <- off_farm_income[1487:5262,]

#------------------------
#Other income (non-labor)
#------------------------
other_inc    <- read.dta("Data/Ethiopia 2013/Household/sect11a_hh_w2.dta")
...
other_income <- arrange(other_income, household_id)
other_income <- other_income[1487:5262,]
...
write.csv(c, "imputed_area2013.csv", row.names=TRUE)
